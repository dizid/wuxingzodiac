...
2. `npm create vue@latest` -- Vue 3 + TypeScript + Router + Pinia + Vitest
...
1. Define all TypeScript types (SignProfile, SignContent, etc.)
2. Create 5 element color palettes in main.css (@theme block)
3. Build `useElementTheme` composable
4. Build base UI components (ElementGlassCard, ElementText, ElementButton, ElementParticles)
5. Set up router with all routes
...
6. Build NavBar with zodiac navigation

### Phase 3: Sign Section Components (Week 3)
1. Build all 10 section components (SignHero → SignHoroscope placeholder)
2. Build MonetizationSlot component
3. Create complete Fire Horse content (adapt from firehorse.info encyclopedia)
4. Wire up end-to-end: directory → sign page → all sections → cross-links
5. Build CompatibilityPage tool

### Phase 4: Content Generation (Week 4-6)
1. Build content generation script using Claude API
2. Generate all 60 sign content files
3. Human review round 1: popular signs (Dragon, Tiger, Horse, Rat, Snake, Monkey)
4. Human review round 2: remaining signs
5. Build and verify 60x60 compatibility matrix
6. Create profiles.ts with all 60 lightweight profiles

### Phase 5: SEO + Monetization (Week 7-8)
1. Add vite-ssg for pre-rendering all pages
2. Auto-generate sitemap.xml at build time
3. Implement dynamic meta tags + JSON-LD per page
4. Set up Google Analytics (GA4)
5. Integrate Google AdSense
6. Add affiliate links in appropriate sections
7. Set up cross-linking between firehorse.info ↔ wuxingzodiac.me
